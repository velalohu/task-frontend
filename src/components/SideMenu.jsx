    ...
    <AnimatePresence>
        {isOpen && (
            ...
            <Motion.div
                ...
                className="fixed left-0 top-0 z-50 h-screen w-64 bg-cardback p-6 text-white shadow-lg"
            >
                    <div className="mb-8 border-b border-white/10 pb-5">
                        <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white/5 text-white">
                            <span className="text-2xl font-bold"> {avatarInitial} </span>
                        </div>
                        <p className="text-xs text-white/60">PANEL DE USUARIO</p>
                        <h2 className="text-xl font-bold"> {userName} </h2>
                    </div>

                    <div className="mb-6 rounded-md border border-white/10 p-3">
                        <p className="mb-2 text-xs text-white/60">SECCIONES</p>
                        <div className="space-y-2">
                            {sectionOptions.map((section) =>
                            <label key={section.key} className="flex cursor-pointer items-center gap-2 text-sm text-white/85">
                                <input type="checkbox" checked={sectionVisibility[section.key]} onChange={() => onToggleSection(section.key)} className="h-4 w-4 accent-white/70" />
                    ...
                    <div className="mb-6 rounded-md border border-white/10 p-3">
                        <p className="mb-2 text-xs text-white/60">PRIORIDAD</p>
                        <div className="space-y-2">
